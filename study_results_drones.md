...
Regression is a statistical technique used to estimate relationships among variables. In most cases the focus is on a 
relationship between dependent and one or more independent variables. The independent variable is 
referred to as the 'predictor' while a dependent variable is the quantity one hopes to predict. In our study a host of 
different quantities might have served as independent variables - soil moisture content, foliar nitrogen level, growing 
days, etc. The goal of predicting yield made the sucrose amount produced at the end of season our dependent variable.   
